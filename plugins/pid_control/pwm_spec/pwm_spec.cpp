...
PWM_SPEC::PWM_SPEC(int dev_addr) {

	//ESP_LOGI("PWM_SPEC", "dev_addr = %d", dev_addr);

	//motor_num = dev_addr;

}

void PWM_SPEC::init(void) {
    mcpwm_config_t pwm_config;

	esp_log_level_set("PWM_SPEC", ESP_LOG_VERBOSE);
...
int PWM_SPEC::prop_count(void) {
	// not supported
	return 0;
}

bool PWM_SPEC::prop_name(int index, char *name) {
	// not supported
	return false;
}

bool PWM_SPEC::prop_unit(int index, char *unit) {
	// not supported
	return false;
}

bool PWM_SPEC::prop_attr(int index, char *attr) {
	// not supported
	return false;
}

bool PWM_SPEC::prop_read(int index, char *value) {
	// not supported
	return false;
}

bool PWM_SPEC::prop_write(int index, char *value) {
	// not supported
	return false;
}
...
void PWM_SPEC::process(Driver *drv) {
    //mcpwm_config_t pwm_config;

	switch (state) {
		case s_detect:
			//ESP_LOGI("PWM_SPEC", "s_detect");
...
void PWM_SPEC::speed(uint8_t dir, int percent) {
    //mcpwm_config_t pwm_config;

	ESP_LOGI("PWM_SPEC", "direction %d speed %d", dir, percent);

	if (percent > 100) percent = 100;
	if (percent < 0) percent = 0;

	gpio_set_level(MOTOR_STRT, 1);
	tickcnt = get_tickcnt(); while (!is_tickcnt_elapsed(tickcnt, 100));
	...
	// mcpwm_set_frequency(MCPWM_UNIT, timer_num, 1000);
	// mcpwm_set_duty(MCPWM_UNIT, timer_num, MCPWM_OPR_A, 50);
	// mcpwm_set_duty_type(MCPWM_UNIT, timer_num, MCPWM_OPR_A, MCPWM_DUTY_MODE_0);
	// gpio_set_level(MOTOR_STRT, 1);
	// tickcnt = get_tickcnt(); while (!is_tickcnt_elapsed(tickcnt, 100));

	uint32_t frq = 230*percent + 1000;
	mcpwm_set_frequency(MCPWM_UNIT, timer_num, frq);
	mcpwm_set_duty(MCPWM_UNIT, timer_num, MCPWM_OPR_A, 50);
	...
	// tickcnt = get_tickcnt();
	// while (!is_tickcnt_elapsed(tickcnt, 200)) {tickcnt = get_tickcnt();};
	// mcpwm_set_frequency(MCPWM_UNIT, timer_num, 5000);
	// gpio_set_level(MOTOR_STRT, 0);

	// tickcnt = get_tickcnt();
	// while (!is_tickcnt_elapsed(tickcnt, 100)) {tickcnt = get_tickcnt();};
	// uint32_t frq = 230*percent + 1000;
	// mcpwm_set_frequency(MCPWM_UNIT, timer_num, frq);
}

void PWM_SPEC::stop(void) {
