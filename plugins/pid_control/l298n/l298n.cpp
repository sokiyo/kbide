...
L298N::L298N(int dev_addr) {

	//ESP_LOGI("L298N", "dev_addr = %d", dev_addr);

	motor_num = dev_addr;

}

void L298N::init(void) {
    mcpwm_config_t pwm_config;

	esp_log_level_set("L298N", ESP_LOG_VERBOSE);
...
int L298N::prop_count(void) {
	// not supported
	return 0;
}

bool L298N::prop_name(int index, char *name) {
	// not supported
	return false;
}

bool L298N::prop_unit(int index, char *unit) {
	// not supported
	return false;
}

bool L298N::prop_attr(int index, char *attr) {
	// not supported
	return false;
}

bool L298N::prop_read(int index, char *value) {
	// not supported
	return false;
}

bool L298N::prop_write(int index, char *value) {
	// not supported
	return false;
}
...
void L298N::process(Driver *drv) {
    //mcpwm_config_t pwm_config;

	switch (state) {
		case s_detect:
			//ESP_LOGI("L298N", "s_detect");
...
void L298N::speed(uint8_t dir, int percent) {
    mcpwm_config_t pwm_config;

	//ESP_LOGI("L298N", "direction %d speed %d", dir, percent);
