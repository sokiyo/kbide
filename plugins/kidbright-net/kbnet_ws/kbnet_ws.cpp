...
static void IRAM_ATTR kbnet_ws_gpio_isr_handler(void* arg) {
	KBNET_WS *kbnet_ws = (KBNET_WS *)arg;
    int64_t ts = esp_timer_get_time(); // int64_t = 2^63÷10^6÷(24×60×60)÷365 = 292471.208677536 years

	// wind speed = 2.4 km/h at 1 Hz
	kbnet_ws->wind_speed = (2.4 * 1000000) / (ts - kbnet_ws->last_wind_speed_ts);
	kbnet_ws->last_wind_speed_ts = ts;

	// update tickcnt
	kbnet_ws->tickcnt = kbnet_ws->get_tickcnt();
}

KBNET_WS::KBNET_WS() {
	// set sensor gpio
	address = IN1_GPIO;
	sensor_gpio = (gpio_num_t)IN1_GPIO;
}

void KBNET_WS::init(void) {
	last_wind_speed_ts = 0;
	wind_speed = 0;
	state = s_detect;
}

int KBNET_WS::prop_count(void) {
	return 1;
}

bool KBNET_WS::prop_name(int index, char *name) {
	if (index == 0) {
...
bool KBNET_WS::prop_unit(int index, char *unit) {
	if (index == 0) {
...
bool KBNET_WS::prop_attr(int index, char *attr) {
	if (index == 0) {
...
bool KBNET_WS::prop_read(int index, char *value) {
	if (index == 0) {
...
bool KBNET_WS::prop_write(int index, char *value) {
	// not supported
	return false;
}

void KBNET_WS::process(Driver *drv) {
	...
	switch (state) {
		case s_detect:
			...
			gpio_isr_handler_add(sensor_gpio, kbnet_ws_gpio_isr_handler, (void *)this);
...
double KBNET_WS::get_wind_speed_km_h(void) {
	double val;
...
char *KBNET_WS::get_wind_speed_km_h_text(int precision) {
	char fmt[8];
	...
	snprintf(wind_speed_text, sizeof(wind_speed_text) - 1, fmt, wind_speed);
