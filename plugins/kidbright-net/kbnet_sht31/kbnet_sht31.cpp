...
#include "kbnet_sht31.h"

#undef __DEBUG__

#ifdef __DEBUG__
#include "esp_log.h"
static const char *TAG = "kbnet_sht31";
#endif

// sht31 registers
#define KBNET_SHT31_MEAS_HIGHREP         0x2400

KBNET_SHT31::KBNET_SHT31(void) {
	// set sensor i2c channel and address
	channel = 0;
	address = 0x44;
	polling_ms = KBNET_SHT31_POLLING_MS;
}

void KBNET_SHT31::init(void) {
	temperature = 0;
	humidity = 0;
	state = s_detect;
}

int KBNET_SHT31::prop_count(void) {
	return 2;
}

bool KBNET_SHT31::prop_name(int index, char *name) {
	if (index == 0) {
...
bool KBNET_SHT31::prop_unit(int index, char *unit) {
	if (index == 0) {
...
bool KBNET_SHT31::prop_attr(int index, char *attr) {
	if ((index == 0) || (index == 1)) {
...
bool KBNET_SHT31::prop_read(int index, char *value) {
	if (index == 0) {
...
bool KBNET_SHT31::prop_write(int index, char *value) {
	// not supported
	return false;
}

uint8_t KBNET_SHT31::crc8(const uint8_t *data, int size) {
...
void KBNET_SHT31::process(Driver *drv) {
	...
	switch (state) {
		case s_detect:
			...
			if (i2c->detect(channel, address) == ESP_OK) {
				...
				data[0] = KBNET_SHT31_MEAS_HIGHREP >> 8;
				data[1] = KBNET_SHT31_MEAS_HIGHREP & 0xff;
...
double KBNET_SHT31::get_temperature(void) {
	return temperature;
}

double KBNET_SHT31::get_humidity(void) {
	return humidity;
}
