...
public:
	...
	uint32_t age() const { return valid ? esp_timer_get_time() - lastCommitTime : (uint32_t)ULONG_MAX; }
...
public:
	bool isValid() const { return valid; }
	bool isUpdated() const { return updated; }
	int64_t age() const { return valid ? esp_timer_get_time() - lastCommitTime : (uint32_t)ULONG_MAX; }
	uint32_t value()
	{
		updated = false;
		return val;
