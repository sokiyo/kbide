...
Blockly.JavaScript['math_variables_set'] = function(block) {
	...
	// return 'NUMBER_' + varName + ' = ' + argument0 + ';\n';
};
...
Blockly.JavaScript['logic_compare'] = function(block) {
	...
	if (block.childBlocks_[0].outputConnection.check_ === null){
		code = argument0 + ' ' + operator + ' ' + argument1;
	}
	else if (block.childBlocks_[0].outputConnection.check_[0] == 'String') {
...
Blockly.JavaScript['wait_sw1_pressed'] = function(block) {
	var code = 'while(1){if ((get_B1state() == 1 ) || (get_B1state() == 2 ) || button12.is_sw1_pressed()){if(get_B1state() == 2){set_B1release();} break;} vTaskDelay(50 / portTICK_RATE_MS);}\n';
	// button12.wait_sw1_pressed();\n
	return code;
};

Blockly.JavaScript['wait_sw1_released'] = function(block) {
	var code = 'while(1){if ((get_B1state() == 0 ) && button12.is_sw1_released()){break;} vTaskDelay(50 / portTICK_RATE_MS);}\n';
	// return 'button12.wait_sw1_released();\n';
	return code;
};

Blockly.JavaScript['wait_sw2_pressed'] = function(block) {
	var code = 'while(1){if ((get_B2state() == 1 ) || (get_B2state() == 2 ) || button12.is_sw2_pressed()){if(get_B2state() == 2){set_B2release();} break;} vTaskDelay(50 / portTICK_RATE_MS);}\n';
	// return 'button12.wait_sw2_pressed();\n';
	return code;
};

Blockly.JavaScript['wait_sw2_released'] = function(block) {
	var code = 'while(1){if ((get_B2state() == 0 ) && button12.is_sw2_released()){break;} vTaskDelay(50 / portTICK_RATE_MS);}\n';
